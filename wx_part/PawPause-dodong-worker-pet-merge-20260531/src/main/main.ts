...
const { app, BrowserWindow, ipcMain, Menu, nativeTheme, net, protocol, screen, session, shell, Tray } = electron;
...
const HEALTH_BRIDGE_CHECK_INTERVAL_MS = 5000;
const HEALTH_BRIDGE_MAX_EVENT_AGE_MS = 10 * 60 * 1000;
const WORKER_PET_MODE = true;
...
type HealthBridgeAction = "exercise" | "break" | "none";
type HealthBridgeSeverity = "info" | "success" | "warning" | "critical";
type HealthBridgeEventType =
  | "meeting_start"
  | "meeting_end"
  | "work_active"
  | "analyzing"
  ...
  | "exercise_done"
  | string;
type BanweiLevel = "low" | "medium" | "high";
type HealthBridgePayload = {
  banweiLevel?: BanweiLevel;
  banweiScore?: number;
  message?: string;
  suggestedAction?: HealthBridgeAction;
};
type HealthBridgeEvent = {
  id?: string;
  type?: HealthBridgeEventType;
  source?: string;
  title?: string;
  message?: string;
  severity?: HealthBridgeSeverity;
  action?: HealthBridgeAction;
  state?: PetState;
  timestampMs?: number;
  timestamp?: string;
  payload?: HealthBridgePayload;
};
type HealthEventFileCursor = {
  size: number;
  partialLine: string;
};
...
let healthBridgeTimer: NodeJS.Timeout | null = null;
...
function getSettings(): Settings {
  ...
  return {
    ...
    petScale: WORKER_PET_MODE ? 0.7 : normalizePetScale(stored.petScale),
    petRoamEnabled:
    ...
    breakReminderEnabled: WORKER_PET_MODE ? false : Boolean(stored.breakReminderEnabled),
    screenBlockReminderEnabled: WORKER_PET_MODE ? false : Boolean(stored.screenBlockReminderEnabled),
    ...
    hydrationReminderEnabled: WORKER_PET_MODE ? false : Boolean(stored.hydrationReminderEnabled),
    ...
    healthBridgeEnabled:
      typeof stored.healthBridgeEnabled === "boolean"
        ? stored.healthBridgeEnabled
        : DEFAULT_SETTINGS.healthBridgeEnabled,
    ...
    distractionDetectionEnabled: WORKER_PET_MODE ? false : Boolean(stored.distractionDetectionEnabled),
    customReminders: WORKER_PET_MODE ? [] : normalizeCustomReminders(stored.customReminders),
    selectedPetId,
    installedPets
  };
}
...
function setSettings(next: Settings): void {
  const normalized = {
    ...
    breakReminderEnabled: WORKER_PET_MODE ? false : Boolean(next.breakReminderEnabled),
    screenBlockReminderEnabled: WORKER_PET_MODE ? false : Boolean(next.screenBlockReminderEnabled),
    ...
    hydrationReminderEnabled: WORKER_PET_MODE ? false : Boolean(next.hydrationReminderEnabled),
    customReminders: WORKER_PET_MODE ? [] : normalizeCustomReminders(next.customReminders),
    healthBridgeEnabled: Boolean(next.healthBridgeEnabled),
...
function loadRenderer(win: Electron.BrowserWindow, route: RendererRoute): void {
  const devServer = process.env.ELECTRON_RENDERER_URL;
  if (devServer) {
...
function createExerciseWindow(): void {
  ...
  if (!meetingCompactActive) {
    ensurePetWindowVisible();
    hideBubble();
    setPetState("exerciseRunning");
    publishSnapshot();
  }

  exerciseWindow = new BrowserWindow({
    width: EXERCISE_WINDOW.width,
    height: EXERCISE_WINDOW.height,
    title: "PawPause Neck Exercise",
    resizable: true,
    minWidth: 760,
    minHeight: 560,
    show: false,
  ...
  exerciseWindow.on("closed", () => {
    exerciseWindow = null;
    if (!meetingCompactActive && petState === "exerciseRunning") {
      setPetState(healthReturnState());
      publishSnapshot();
    }
  });
}
...
function exerciseLabel(): string {
  return getSettings().language === "zh-CN" ? "颈椎操" : "Neck exercise";
}
...
function showPetContextMenu(): void {
  ...
  const labels = text().menu;
  const template: Electron.MenuItemConstructorOptions[] = [
    { label: exerciseLabel(), click: createExerciseWindow },
    { label: labels.settings, click: createSettingsWindow },
    { type: "separator" },
    {
...
function healthBridgeEventPaths(): string[] {
  const home = app.getPath("home");
  const paths = [
    process.env.PAWPAUSE_HEALTH_EVENTS,
    join(home, ".local", "share", "pawpause", "health-events.jsonl"),
    join(home, "Library", "Application Support", "PawPause", "health-events.jsonl"),
    join(home, "AppData", "Roaming", "PawPause", "health-events.jsonl")
  ];
  return [...new Set(paths.filter((path): path is string => Boolean(path)))];
}

function ensureHealthBridgeFiles(): string[] {
  return healthBridgeEventPaths().flatMap((filePath) => {
    try {
...
function primeHealthBridgeFiles(paths: string[]): void {
  for (const filePath of paths) {
    try {
      healthEventFileCursors.set(filePath, {
        size: statSync(filePath).size,
        partialLine: ""
      });
    } catch {
      healthEventFileCursors.delete(filePath);
    }
  }
  healthBridgePrimed = true;
}

function readNewHealthEventLines(filePath: string): string[] {
  const stats = statSync(filePath);
  const previous = healthEventFileCursors.get(filePath) ?? { size: 0, partialLine: "" };
  const startAt = stats.size < previous.size ? 0 : previous.size;
  if (stats.size === startAt) return [];

  const length = stats.size - startAt;
  const buffer = Buffer.alloc(length);
  const fd = openSync(filePath, "r");
  try {
    readSync(fd, buffer, 0, length, startAt);
  } finally {
    closeSync(fd);
  }

  const combined = `${previous.partialLine}${buffer.toString("utf8")}`;
  const lines = combined.split(/\r?\n/);
  const partialLine = combined.endsWith("\n") || combined.endsWith("\r") ? "" : (lines.pop() ?? "");
  healthEventFileCursors.set(filePath, { size: stats.size, partialLine });
  return lines.map((line) => line.trim()).filter(Boolean);
}

function healthEventTimestampMs(event: HealthBridgeEvent): number {
...
function normalizeHealthAction(value: unknown): HealthBridgeAction {
  return value === "exercise" || value === "break" || value === "none" ? value : "none";
}

function normalizeBanweiLevel(value: unknown): BanweiLevel {
  return value === "high" || value === "medium" || value === "low" ? value : "low";
}

function healthPayload(event: HealthBridgeEvent): HealthBridgePayload {
  return event.payload && typeof event.payload === "object" ? event.payload : {};
}

function healthEventType(event: HealthBridgeEvent): string {
  return stringValue(event.type);
}

function healthReturnState(): PetState {
  if (meetingCompactActive) return "meetingCompact";
  return workerActivityActive ? "working" : "idle";
}

function enterMeetingCompact(): void {
  meetingCompactActive = true;
  ensurePetWindowVisible();
  hideBubble();
  setPetScaleOverride(0.35);
  setPetState("meetingCompact");
  publishSnapshot();
}

function exitMeetingCompact(): void {
  meetingCompactActive = false;
  setPetScaleOverride(null);
  setPetState(workerActivityActive ? "working" : "idle");
  publishSnapshot();
}

function showTemporaryHealthState(state: PetState, message: string, autoDismissMs = 2600): void {
  ensurePetWindowVisible();
  stopAmbientRoam(false);
  stopAmbientPose(true);
  clearAgentPose(true);
  setPetState(state);
  showBubble({
    id: `health-${state}-${Date.now()}`,
    message,
    autoDismissMs
  });
  setTimeout(() => {
    if (!blockingMode && !meetingCompactActive && petState === state) {
      hideBubble();
      setPetState(healthReturnState());
      if (!workerActivityActive) {
        scheduleAmbientRoam();
        scheduleAmbientPose();
      }
    }
...
function healthActionLabel(action: HealthBridgeAction): string {
  const isChinese = getSettings().language === "zh-CN";
  if (action === "exercise") return isChinese ? "跟我活动一下" : "Move with me";
  if (action === "break") return isChinese ? "休息一下" : "Take a break";
  return isChinese ? "知道了" : "Got it";
}

function banweiDefaultMessage(level: BanweiLevel, score?: number): string {
  const scoreText = typeof score === "number" && Number.isFinite(score) ? `${Math.round(score)} 分` : "有点";
  if (level === "high") {
    return pick([
      `班味 ${scoreText}。你快和工位合体了。`,
      `${scoreText}，高危。颈椎已经在抗议。`,
      "这个班味，多栋闻了都想请年假。"
    ]);
  }
  if (level === "medium") {
    return pick([
      `班味 ${scoreText}，工位封印开始生效了。`,
      `${scoreText}。你和椅子的关系是不是太稳定了？`,
      "多栋建议：现在站起来，保住一点人类感。"
    ]);
  }
  return pick([
    `班味 ${scoreText}刚冒头，伸个懒腰还能救。`,
    `${scoreText}不严重，但多栋已经闻到了。`,
    "班味轻微泄漏，建议假装活动一下。"
  ]);
}

function banweiState(level: BanweiLevel): PetState {
  if (level === "high") return "banweiHigh";
  if (level === "medium") return "banweiMedium";
  return "banweiLow";
}

function banweiMessage(event: HealthBridgeEvent): string {
  ...
  return banweiDefaultMessage(normalizeBanweiLevel(payload.banweiLevel), payload.banweiScore);
}

function neckGuideMessage(event: HealthBridgeEvent): string {
  const payload = healthPayload(event);
  if (typeof payload.message === "string" && payload.message.trim()) return payload.message.trim();
  if (typeof event.message === "string" && event.message.trim()) return event.message.trim();
  return pick([
...
function exerciseDoneMessage(event: HealthBridgeEvent): string {
  const payload = healthPayload(event);
  if (typeof payload.message === "string" && payload.message.trim()) return payload.message.trim();
  if (typeof event.message === "string" && event.message.trim()) return event.message.trim();
  return pick([
...
function healthEventId(event: HealthBridgeEvent, message: string, timestampMs: number): string {
  if (typeof event.id === "string" && event.id.trim()) return event.id.trim();
  return `health:${event.source ?? "local"}:${healthEventType(event)}:${timestampMs}:${hashText(message)}`;
}

function rememberHealthEvent(id: string): void {
  healthSeenEventIds.add(id);
  if (healthSeenEventIds.size <= 300) return;
  const first = healthSeenEventIds.values().next().value;
...
function notifyBanweiResult(event: HealthBridgeEvent, id: string): void {
  ...
  const payload = healthPayload(event);
  const level = normalizeBanweiLevel(payload.banweiLevel);
  const message = banweiMessage(event);
  rememberHealthEvent(id);
  ensurePetWindowVisible();
  stopAmbientRoam(false);
  stopAmbientPose(true);
  clearAgentPose(true);
  setPetState(banweiState(level));
  showBubble({
    id,
    message,
    autoDismissMs: 5000
...
function notifyNeckGuide(event: HealthBridgeEvent, id: string): void {
  if (meetingCompactActive) return;
  rememberHealthEvent(id);
  ensurePetWindowVisible();
  stopAmbientRoam(false);
...
function notifyExerciseDone(event: HealthBridgeEvent, id: string): void {
  ...
  meetingCompactActive = false;
  setPetScaleOverride(null);
  showTemporaryHealthState("exerciseDone", exerciseDoneMessage(event));
}

function notifyLegacyHealthEvent(event: HealthBridgeEvent, id: string): void {
  if (meetingCompactActive) return;
  const action = normalizeHealthAction(event.action ?? healthPayload(event).suggestedAction);
  const message = banweiMessage(event);
  rememberHealthEvent(id);
  ensurePetWindowVisible();
  stopAmbientRoam(false);
  stopAmbientPose(true);
  clearAgentPose(true);
  setPetState(action === "exercise" ? "neckGuide" : "banweiLow");
  showBubble({
    id,
    message,
    actions:
      action === "none"
        ? undefined
        : [{ id: `health:${action}`, label: healthActionLabel(action), kind: "primary" }],
    autoDismissMs: action === "none" ? 5000 : undefined
  });
}

function notifyHealthBridgeEvent(event: HealthBridgeEvent): void {
  if (blockingMode === "breakRun" || blockingMode === "focusWarning") return;
  const type = healthEventType(event);
  const message = type || banweiMessage(event);
  const timestampMs = healthEventTimestampMs(event);
  const id = healthEventId(event, message, timestampMs);
  if (healthSeenEventIds.has(id)) return;
  if (Date.now() - timestampMs > HEALTH_BRIDGE_MAX_EVENT_AGE_MS) {
    rememberHealthEvent(id);
    return;
  }
  ...
  if (type === "analyzing") {
    rememberHealthEvent(id);
    showTemporaryHealthState("analyzing", healthPayload(event).message ?? event.message ?? "多栋看一眼班味浓度。");
    return;
  }
  if (type === "banwei_result") {
    notifyBanweiResult(event, id);
    return;
  }
  if (type === "neck_guide") {
    notifyNeckGuide(event, id);
    return;
  }
  if (type === "exercise_done") {
    notifyExerciseDone(event, id);
    return;
  }
  notifyLegacyHealthEvent(event, id);
}

function parseHealthBridgeLine(line: string): HealthBridgeEvent | null {
  try {
    const parsed = JSON.parse(line) as unknown;
    if (!parsed || typeof parsed !== "object") return null;
    return parsed as HealthBridgeEvent;
  } catch {
    return { message: line, severity: "info", action: "none", timestampMs: Date.now() };
  }
}

function checkHealthBridgeNow(): void {
  if (!getSettings().healthBridgeEnabled) return;
  const paths = ensureHealthBridgeFiles();
  if (!healthBridgePrimed) {
    primeHealthBridgeFiles(paths);
    return;
  }

  for (const filePath of paths) {
    try {
      for (const line of readNewHealthEventLines(filePath)) {
        const event = parseHealthBridgeLine(line);
        if (event) notifyHealthBridgeEvent(event);
      }
    } catch {
      healthEventFileCursors.delete(filePath);
    }
...
function scheduleHealthBridgeMonitor(): void {
  if (healthBridgeTimer) {
    clearInterval(healthBridgeTimer);
    healthBridgeTimer = null;
  }
  if (!getSettings().healthBridgeEnabled) return;
  healthBridgeTimer = setInterval(checkHealthBridgeNow, HEALTH_BRIDGE_CHECK_INTERVAL_MS);
  checkHealthBridgeNow();
}
...
function triggerWorkerDemo(state: WorkerDemoState): void {
  workerDemoPanelOpen = false;
  ensurePetWindowVisible();
  if (state === "idle") {
    workerActivityActive = false;
    meetingCompactActive = false;
  ...
  if (state === "banweiLow") {
    notifyBanweiResult(workerDemoEvent("banwei_result", { banweiLevel: "low", banweiScore: 28 }), `demo-banwei-low-${Date.now()}`);
    return;
  }
  if (state === "banweiMedium") {
    notifyBanweiResult(workerDemoEvent("banwei_result", { banweiLevel: "medium", banweiScore: 67 }), `demo-banwei-medium-${Date.now()}`);
    return;
  }
  if (state === "banweiHigh") {
    notifyBanweiResult(workerDemoEvent("banwei_result", { banweiLevel: "high", banweiScore: 91 }), `demo-banwei-high-${Date.now()}`);
    return;
  }
  if (state === "neckGuide") {
    notifyNeckGuide(workerDemoEvent("neck_guide", { suggestedAction: "exercise" }), `demo-neck-guide-${Date.now()}`);
    return;
  }
  if (state === "exerciseDone") {
    notifyExerciseDone(workerDemoEvent("exercise_done"), `demo-exercise-done-${Date.now()}`);
  }
}
...
function handleBubbleAction(actionId: string): void {
  ...
  if (actionId === "health:exercise") {
    createExerciseWindow();
    return;
  }
  if (actionId === "health:break") {
    triggerBreakReminder(true);
    return;
  }
  if (actionId === "health:hydration") {
    triggerHydrationReminder(true);
    return;
  }
  if (actionId === "health:focus") {
    startFocusMode();
    return;
  }
...
app.whenReady().then(() => {
  session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
    callback(permission === "media" && exerciseWindow?.webContents.id === webContents.id);
  });
