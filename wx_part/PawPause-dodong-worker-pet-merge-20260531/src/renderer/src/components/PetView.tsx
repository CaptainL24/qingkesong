...
const workerDemoStates: Array<{ state: WorkerDemoState; label: string }> = [
  { state: "idle", label: "普通" },
  { state: "meetingCompact", label: "会议球" },
  { state: "working", label: "陪工" },
  { state: "analyzing", label: "分析" },
  { state: "banweiLow", label: "低班味" },
  { state: "banweiMedium", label: "中班味" },
  { state: "banweiHigh", label: "高班味" },
...
export function PetView(): JSX.Element {
  ...
  function clearPetClickTimer(): void {
  ...
  useEffect(() => {
    if (lyricsModeEnabled && demoPanelOpen) {
      setDemoPanelOpen(false);
      return;
    }
    window.pawpause.setWorkerDemoPanelOpen(demoPanelOpen);
  ...
  return (
    ...
    >
      ...
      {demoPanelOpen && !lyricsModeEnabled ? (
        <section className="pet-demo-panel" aria-label="比格多栋状态演示">
          <div className="pet-demo-panel__header">
            <span>现场演示</span>
            <button
              aria-label="关闭演示面板"
              className="pet-demo-panel__close"
          ...
          <div className="pet-demo-panel__grid">
            {workerDemoStates.map((item) => (
              <button
                className={`pet-demo-panel__button${state === item.state ? " is-active" : ""}`}
                key={item.state}
                onClick={() => {
                  setDemoPanelOpen(false);
                  window.pawpause.triggerWorkerDemo(item.state);
                }}
                type="button"
              >
                {item.label}
      ...
      <button
        ...
        type="button"
      >
        {asset.kind === "stateImage" ? (
          <img className="state-image-pet" src={asset.src} alt={altText} draggable={false} />
        ) : asset.kind === "sprite" ? (
          <span
          ...
          <span
            className={`fallback-pet fallback-pet--${selectedPetId === "dodong" ? "dodong" : "default"}`}
            role="img"
            aria-label={altText}
          >
