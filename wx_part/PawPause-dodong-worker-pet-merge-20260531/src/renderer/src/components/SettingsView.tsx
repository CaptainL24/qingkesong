    ...
    return (
      <main className="prefs" dir={language === "ar" ? "rtl" : "ltr"}>
        <header className="prefs__topbar">
          <h1 className="prefs__page-title worker-title">打工人桌宠</h1>
          <button type="button" className="pref-button is-primary" onClick={window.pawpause.openExercise}>
            {language === "zh-CN" ? "颈椎操" : "Neck exercise"}
          </button>
        </header>
        ...
        <section className="prefs__group">
          <h2 className="prefs__group-title">Health Bridge</h2>
          <Row
            label={language === "zh-CN" ? "启用本地事件入口" : "Enable local event bridge"}
            hint={
              language === "zh-CN"
            ...
            control={
              <ToggleControl
                checked={draft.healthBridgeEnabled}
                onChange={(healthBridgeEnabled) => updateDraft({ healthBridgeEnabled })}
                ariaLabel="Health Bridge"
              />
            }
          />
        </section>
