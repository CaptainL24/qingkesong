...
export function mapPetStateToPetdexState(state: PetState): PetdexStateId {
  ...
  if (state === "meetingCompact") return "idle";
  if (state === "working") return "waiting";
  if (state === "analyzing") return "review";
  if (state === "banweiLow") return "waving";
  if (state === "banweiMedium") return "review";
  if (state === "banweiHigh") return "failed";
  if (state === "neckGuide") return "running";
  if (state === "exerciseRunning") return "running";
  if (state === "exerciseDone") return "jumping";
